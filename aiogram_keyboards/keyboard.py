...
class Keyboard(metaclass=Meta):
    ...
    def __init_subclass__(cls, **kwargs):
        ...
        if cls.handler != Keyboard.handler:
            async def handler(meta: DialogMeta):
                obj = cls()

                if isinstance(obj, DialogContext):
                    obj = await obj.__aenter__(meta)

                await obj.handler(meta)
    ...
    async def process(cls,
                      obj: Union[Message, CallbackQuery, DialogMeta],
                      markup_type: str = None, /,
                      visualize: bool = True) -> Message:
        ...
        result = await cls.__core__.process(obj, markup_type, visualize)
