...
from aiogram.types import InlineKeyboardMarkup, ReplyKeyboardMarkup, InlineKeyboardButton, ParseMode, Message
from aiogram.utils.exceptions import MessageCantBeEdited, MessageToEditNotFound, BadRequest
...
class RuntimeOptions:
    def __init__(self, remove=None, select=None):
        self.remove: list[int] = remove or []
        self.select: Optional[int] = select
...
class Runtime:
    ...
    Help you configure runtime behavior of process method.
    All methods set preferences in next process in selected scope.
    ...
    @classmethod
    async def get_data(cls, meta: DialogMeta) -> RuntimeOptions:
        data = await meta.fsm_context.get_data()
        return RuntimeOptions(**data.get(cls.KW, dict()))

    @classmethod
    async def reset_data(cls, meta: DialogMeta):
        return await cls.save_data(meta, RuntimeOptions())

    @classmethod
    async def save_data(cls, meta: DialogMeta, data_to_save: RuntimeOptions):
        data = await meta.fsm_context.get_data()
        data.update({cls.KW: data_to_save.json()})
        await meta.fsm_context.set_data(data)

    @classmethod
    async def remove(cls, meta: DialogMeta, message_id: int = None):
        """ Remove message """

        runtime = await cls.get_data(meta)
        runtime.remove.append(message_id)
        await cls.save_data(meta, runtime)

    @classmethod
    async def select(cls, meta: DialogMeta, message_id: int = None):
        """ Work with spec message """

        runtime = await cls.get_data(meta)
        runtime.select = message_id
        await cls.save_data(meta, runtime)
...
class Markup:
    ...
    def rows(self):
        ...
        for i in range(len(tuple(filter(lambda x: x.text, self.buttons)))):
            if i % self.width == 0:
                rows.append([])
    ...
    async def process(self,
                      ...
                      markup_scope: Literal['m', 'c', 'm+c'] = None,
                      visualize: bool = True) -> Optional[Message]:
        ...
        :param visualize: is process real object in chat
        ...
        if not isinstance(raw_meta, DialogMeta):
            meta = DialogMeta(raw_meta)
        else:
            meta = raw_meta
            raw_meta = meta.source
        ...
        runtime_data = await Runtime.get_data(meta)
        await Runtime.reset_data(meta)
            ...
            if markup_type == MarkupType.TEXT:
                ...
                try:
                    response = await dp.bot.edit_message_text(chat_id=meta.chat_id,
                                                              text=text,
                                                              reply_markup=reply_markup,
                                                              message_id=runtime_data.select or meta.active_message_id,
                                                              parse_mode=ParseMode.HTML)

                except (MessageCantBeEdited, MessageToEditNotFound, BadRequest):
                    response = await dp.bot.send_message(chat_id=meta.chat_id,
                                                         text=text,
                                                         reply_markup=reply_markup,
                                                         parse_mode=ParseMode.HTML)
        ...
        for i in runtime_data.remove:
            try:
                await dp.bot.delete_message(chat_id=meta.chat_id,
                                            message_id=i)
            except BadRequest:
                logger.debug(f'Error while remove message with id {i}')
